        ...
        steps {
            script {
                ...
                withCredentials([file(credentialsId: "${GCP_PROJECT_ID}-jenkins-keys", variable: "SERVICE_ACCOUNT")]) {
                    ...
                    sh "gcloud beta run deploy --image=${DOCKER_REGISTRY}/${PROJECT_NAME}/${GIT_REPO}:${COMMIT7} --set-cloudsql-instances ${instance_name} --set-env-vars ENV=production,GCP_PROJECT=${GCP_PROJECT_ID} --platform managed"
                }
            }
        }
    }
